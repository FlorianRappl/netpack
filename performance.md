...
## Mixed JS/TS Module Stress Case

For profiling the hot path behind large minified bundles, use the synthetic case
in [src/NetPack.Tests/PerformanceStressTests.cs](src/NetPack.Tests/PerformanceStressTests.cs).
It generates 96 alternating `.ts` and `.js` modules, then measures graph build
time, pretty emission time, and minified emission time separately.

Run it with:

```sh
dotnet test src/NetPack.Tests/NetPack.Tests.csproj --filter FullyQualifiedName~PerformanceStressTests -v normal
```

The test logs the module count, bundle count, stage timings, and the pretty vs
minified output sizes. That makes it easy to tell whether the bottleneck is in
resolution/traversal or in compact emission.

Likely follow-up optimizations to inspect if this case is slow:

- Cache or reuse file-resolution results within a single traversal.
- Reduce temporary allocations in `Mangler.Process` for large ASTs.
- Avoid repeated string-building work in the compact printer for large bundles.
- Profile extensionless JS/TS resolution separately from printing, since the
  large-module case exercises both heavily.
